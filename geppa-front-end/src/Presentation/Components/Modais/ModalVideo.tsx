...
const ModalArtigo: React.FC<ModalConteudoProps> = ({ abrir, fechar, mostrar, salvar, video }) => {
    ...
    useEffect(() => {
        if (video) {
            ...
            setLink(video.link)
            setDataCadastro(new Date(video.dataCadastro))
            setTags(video.tags || []);
        }
    }, [video]);
    ...
    return (
        <>
            ...
            <Modal show={abrir} onHide={cancelar} backdrop="static">
                <Modal.Header>
                    <Modal.Title>{video ? 'Editar' : 'Adicionar'}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <form>
                        ...
                        <label htmlFor="dataCadastro">Quando o vídeo foi lançado? *</label>
                        <input
                            type="date"
                            className="form-control"
                            id="dataCadastro"
                            value={dataCadastro ? dataCadastro.toISOString().substr(0, 10) : ''}
                            onChange={(e) => setDataCadastro(new Date(e.target.value))}
                        />
